...
export const addCardToList = (cards: Card[], index: number, card: Card): Card[] =>
  cards.slice(0, index).concat(card).concat(cards.slice(index));
...
export const updateListCards = (lists: List[], listId: string, updateFn: (cards: Card[]) => Card[]): List[] =>
  lists.map(list => list.id === listId ? { ...list, cards: updateFn([...list.cards]) } : list);
